...
The goal of this post is to show show workflow for forecasting time dependent data in R. The forecasting methodologies conducted in the post are on time series where data is sequential and have dependency structure. Sometimes we are more interested in having accurate forecast.
...
Many of the methods were historically deveoped from 1960s and have analytical form (e.g. ARIMA), whereas the more recent methods (e.g. RNN) are using machine learning and neural network without precise analytical formula for estimators. 

# Workflow

I. Time objects in R and Data Preprocessing
...
II. EDA and Visualization
  ...
  - Location, Dispersion, and Distribution of Univariate time series
...
V. Models
  ...
  - Univariate time series -> can apply any models from A-J
  - Univariate time series with exogenous variables -> can only apply B, K
  - Multiple time series 
  - Hierarchical time series
  - Mulitple time series with exogenous variables
...
library(dplyr) # data manpluation
...
4. Gas CO2 emission
5. CME 1: seat price
6. CME 2: trading volume
7. gold, oil, 

# I. Preprocessing: Date Objects in R

Date is a special type of data in R.

### R's base Date object

benefit of date object is we can work with it more easily. dates act like numbers ; inequality relation holds.

in ggplot you can specify the limits by xlim or scale_x_Date functions

can apply max() min()

R to recognize a string as a datetime you need to convert it, although now you use as.POSIXct(). as.POSIXct() expects strings to be in the format YYYY-MM-DD HH:MM:SS.
...
Strengths of lubridate library:
...
Timespans are hard because they dont hve constant meaning. 
...
ou can create an interval by using the operator %--% with two datetimes. For example ymd("2001-01-01") %--% ymd("2001-12-31") creates an interval for the year of 2001.
...
Graphs enable you to visualize many features of the data, including patterns, unusual observations, changes over time, and relationships between variables. Just as the type of data determines which forecasting method to use, it also determines which graphs are appropriate.
...
Time Series decomposition into trend-cycle, seasonal, and remainder
...
### 2. Classical Decomposition: Additive and Mltiplicative
```{r}
...
To forecast a decomposed time series, we forecast the seasonal component,  and the seasonally adjusted component  imated component. In other words, a seasonal naïve method is used for the seasonal component.
...
shows naïve forecasts of the seasonally adjusted electrical equipment orders data. These are then "reseasonalised" by adding in the seasonal naïve forecasts of the seasonal component.
...
As long as they have zero mean point forecasts re okay. But when normality and autocorrelation are not satisfied, confidence intervals are not relaiable
...
Traiditonal evaluation

Time series cross-validation: series of training and test sets. forecastingg on rolling points.
